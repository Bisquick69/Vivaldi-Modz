...
var webstackChangeObserver = new MutationObserver(function (
	mutations,
	observer
) {
...
function addNewPaneButton () {
	...
	newButton.title = "Extensionz";
	newButton.tabindex = "0";
	newButton.className = "button-startpage";
	newButton.innerHTML = "Extensionz Pane";
	newButton.id = "ExtPaneButton";
	var navbar = document.querySelector(
		"nav.startpage-navigation > div:nth-child(2)"
	);
...
function registerSwitchbackObserver () {
	var switchbackObserver = new MutationObserver(function (mutations, observer) {
		mutations.forEach(function (mutation) {
			if (
				mutation.attributeName === "class" &&
				mutation.target.className.indexOf("active") >= 0 &&
...
function activate (e) {
	...
	container.innerHTML = "<div class='newpane manager'>Extensionz</div>";
}
